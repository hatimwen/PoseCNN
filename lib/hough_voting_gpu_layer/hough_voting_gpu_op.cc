...
class HoughvotinggpuOp<Eigen::GpuDevice, T> : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override 
  {
    ...
    reset_outputs(top_box, top_pose, top_target, top_weight, top_domain, num_rois_device, num_classes, hough_space);
    ...
    for (int n = 0; n < batch_size; n++)
    {
      ...
      HoughVotingLaucher(context, labelmap, vertmap, extents, meta_data, gt, cls_loss, n, batch_size, height, width, num_classes, num_gt,
        ...
        top_box, top_pose, top_target, top_weight, top_domain, num_rois_device, context->eigen_device<Eigen::GpuDevice>(), hough_space);
    }
    ...
    copy_outputs(top_box, top_pose, top_target, top_weight, top_domain, hough_space, top_box_final,
      top_pose_final, top_target_final, top_weight_final, top_domain_final, top_hough_space_final, num_classes, num_rois);
  }
 private:
  int is_train_;
  float threshold_vote_;
